...
class RationalParticle:
    ...
    def computeposterior(self, stim, env=None, calcclust=False):
        ...
        for k in range(self.clusters):
            ...
            # Prior:
            pk[k] = self.c * float(sum([item==k for item in self.partition]))/\
    ...
    def getposterior (self, stim):
        ...
        assert stimsequal( self.laststim, self.stims[stim]), \
    ...
    def findMAPval(self, stimulus, env):
        ...
        for i, s in enumerate( self.stims ):
            if stimsequal( stimulus, s ):
                ...
                break
        if stimnum == -1:
            self.stims.append( stimulus ) # WARNING: this is a hackish solution, should be refactored.
        ...
        if self.types[qdim] == 'c':
            ...
            for k in range(len(np.unique(self.partition))):
                pjF.append( np.argmax( [self.valprob( k, qdim, j) for j in range(2)] )  * pkF[k])
...
def testcontinuous():
    ...
    outfile = "/dev/null"
    import pylab as pl
    pl.suptitle( "Six partitions made using a single particle each." )
    for run in range( 6 ):
